...
void *operator new(std::size_t size, void *t) { return t; }

namespace processor {

static constexpr u32 msr_kvm_pv_eoi = 0x4b564d04;
//static u32 kvm_pv_eoi_word[128];

void kvm_pv_eoi_init(int ncpu)
{
    /*if (features().kvm_pv_eoi) {
        wrmsr(msr_kvm_pv_eoi, mmu::virt_to_phys(&kvm_pv_eoi_word[ncpu]) | 1);
    }*/
}
...
private:
    ...
    u8* const _base_virt = phys_to_virt<u8>(_apic_base);
...
xapic::xapic()
    : apic_driver()
{
    mmu::map(mmu::vaddr(_base_virt), _apic_base);
    xapic::enable();
}
...
static bool is_xen() { return false; }
...
apic_driver* create_apic_driver()
{
    ...
    if (features().x2apic) {
        return new (&_x2apic) x2apic;
    } else {
        return new (&_xapic) xapic;
    };
}
